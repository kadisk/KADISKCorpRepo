...
const MyServicesContainer = ({
    ...
    const [ interfaceModeType,  setInterfaceModeType] = useState<any>(LOADING_MODE)
    ...
    const fetchMyServicesStatus = async () => {
        ...
        if(response.data === "READY"){
            setInterfaceModeType(DEFAULT_MODE)
        }
    }
        
    const changeMode = (mode) => setInterfaceModeType(mode)
    const handleImportNew = () => changeMode(IMPORT_SELECT_MODE)
    const handleImportingMode = (importData) => changeMode(IMPORTING_MODE)
